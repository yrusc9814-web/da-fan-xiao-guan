...
import type { DashboardCalendarDay, DashboardDto, DashboardMealSlot, DashboardRecipe } from '../../../shared/types';
...
import randomDiceImage from '../assets/home-actions/random-dice.png';
import mealBowlImage from '../assets/home-actions/meal-bowl.png';
import inventoryBasketImage from '../assets/home-actions/inventory-basket.png';
import recommendClipboardImage from '../assets/home-actions/recommend-clipboard.png';
...
const quickActions = [
  { title: '随机决定', description: '不知道吃什么？\n让运气帮你决定', to: '/recommendations?mode=random', image: randomDiceImage, tone: 'pink' },
  { title: '搭配一顿饭', description: '智能搭配食材\n生成营养均衡一餐', to: '/recommendations?mode=meal-set', image: mealBowlImage, tone: 'green' },
  { title: '库存消耗', description: '优先消耗即将过期\n的食材不浪费', to: '/inventory', image: inventoryBasketImage, tone: 'orange' },
  { title: '库存推荐', description: '根据库存推荐\n合适的菜谱', to: '/recommendations?mode=inventory', image: recommendClipboardImage, tone: 'purple' }
];

const dashboard = computed(() => dashboardStore.data);
const nickname = computed(() => dashboard.value?.userNickname ?? '');
const greeting = computed(() => dashboard.value?.greetingPeriod === 'morning'
  ? '早上好'
  : dashboard.value?.greetingPeriod === 'afternoon'
    ? '下午好'
    : '晚上好');
const visibleCalendarDays = computed(() => shiftCalendarDays(dashboard.value?.calendarDays ?? [], calendarOffset.value));
...
function uploadedRecipeImage(imagePath: string | null): string | null {
  ...
  const normalized = imagePath.replaceAll('\\', '/').replace(/^\/+/, '').replace(/^uploads\//, '');
...
function shiftCalendarDays(days: DashboardCalendarDay[], offset: number): DashboardCalendarDay[] {
  if (offset === 0) return days;
  return days.map((day) => {
    const date = new Date(`${day.date}T12:00:00`);
    date.setDate(date.getDate() + offset * 7);
    const shifted = date.toISOString().slice(0, 10);
    return { ...day, date: shifted, dayOfMonth: date.getDate(), isToday: false, status: 'empty' };
  });
}
    ...
    <template v-else-if="dashboard">
      <header class="home-hero" aria-labelledby="homepage-title">
        <div class="home-hero__copy">
          <h1 id="homepage-title">{{ greeting }}<template v-if="nickname">，{{ nickname }}</template> <span aria-hidden="true">♥</span></h1>
      ...
      <section class="quick-actions" aria-labelledby="quick-actions-title">
        ...
        <AppCard v-for="action in quickActions" :key="action.title" class="quick-action-card">
          <div class="quick-action-card__copy">
            ...
            <RouterLink :to="action.to" class="app-button app-button--secondary app-button--sm">去{{ action.title.replace('库存', '') }}</RouterLink>
          </div>
          <div class="quick-action-card__visual" :class="`quick-action-card__visual--${action.tone}`">
            <img :src="action.image" :alt="`${action.title}插画`" />
          </div>
        </AppCard>
      </section>
      ...
      <section class="homepage-section homepage-section--recommendations" aria-labelledby="recommendations-title">
        <AppSectionHeader title="今日推荐菜谱" description="从本地菜谱中挑选几道今天的灵感。">
          <template #actions>
            <RouterLink class="section-link" to="/recommendations">查看更多 <AppIcon name="chevron-right" :size="16" /></RouterLink>
          </template>
        </AppSectionHeader>
        <div v-if="dashboard.recommendedRecipes.length" class="recipe-scroller" aria-label="今日推荐菜谱">
          <RouterLink
            ...
            :key="recipe.id"
            class="recipe-card"
            :to="`/recommendations?recipe=${recipe.id}`"
          >
            <div class="recipe-card__image">
              <img :src="resolvedRecipeImage(recipe)" :alt="recipe.name" @error="handleRecipeImageError" />
            </div>
      ...
      <section class="dashboard-info-grid" aria-label="今日概览">
        <AppCard class="dashboard-card today-records-card">
          <AppSectionHeader title="今日饮食记录" description="记录每一餐，慢慢找到适合自己的节奏。">
            <template #actions><span class="section-count">{{ dashboard.todayRecords.filter((meal) => meal.recorded).length }}/3 餐</span></template>
          </AppSectionHeader>
          <div class="meal-slots">
            <div v-for="meal in dashboard.todayRecords" :key="meal.mealType" class="meal-slot" :data-recorded="meal.recorded" :data-meal-type="meal.mealType">
              <span class="meal-slot__icon" aria-hidden="true"><AppIcon :name="mealIcon(meal.mealType)" :size="21" /></span>
          ...
          <RouterLink class="inline-action" to="/records">查看饮食记录 <AppIcon name="arrow-right" :size="14" /></RouterLink>
        </AppCard>

        <AppCard class="dashboard-card weekly-stats-card">
          <AppSectionHeader title="本周统计" description="用轻量数据看见自己的饮食节奏。">
            <template #actions><RouterLink class="section-link" to="/statistics">全部 <AppIcon name="chevron-right" :size="16" /></RouterLink></template>
          </AppSectionHeader>
          <div class="stats-grid">
            <div><span class="stats-grid__icon" aria-hidden="true"><AppIcon name="calendar" :size="22" /></span><strong>记录天数</strong><span>{{ dashboard.weeklyStats.recordedDays }}/7 天</span></div>
            <div><span class="stats-grid__icon" aria-hidden="true"><AppIcon name="clock" :size="22" /></span><strong>总餐次</strong><span>{{ dashboard.weeklyStats.totalMeals }} 餐</span></div>
            <div><span class="stats-grid__icon" aria-hidden="true"><AppIcon name="star" :size="22" /></span><strong>平均评分</strong><span>{{ dashboard.weeklyStats.averageRating ?? '—' }} 分</span></div>
            <div><span class="stats-grid__icon" aria-hidden="true"><AppIcon name="ingredient" :size="22" /></span><strong>食材消耗</strong><span>{{ dashboard.weeklyStats.consumedIngredientCount }} 种</span></div>
          </div>
        </AppCard>

        <AppCard class="dashboard-card inventory-card">
          <AppSectionHeader title="库存概览" description="掌握食材状态，减少临期浪费。">
            <template #actions><RouterLink class="section-link" to="/inventory">全部 <AppIcon name="chevron-right" :size="16" /></RouterLink></template>
          </AppSectionHeader>
          <div class="inventory-metrics">
            <div class="inventory-metric inventory-metric--pink"><strong>{{ dashboard.inventory.totalIngredients }}</strong><span>食材总数</span></div>
            <div class="inventory-metric inventory-metric--orange"><strong>{{ dashboard.inventory.expiringSoon }}</strong><span>即将过期</span></div>
            <div class="inventory-metric inventory-metric--purple"><strong>{{ dashboard.inventory.insufficient }}</strong><span>库存不足</span></div>
          </div>
        ...
        <AppSectionHeader title="饮食日历" description="看看这一周的计划和记录。">
          <template #actions>
            <div class="calendar-actions">
              <button type="button" aria-label="上一周" @click="calendarOffset -= 1"><AppIcon name="chevron-left" :size="18" /></button>
              <span>{{ calendarRange }}</span>
              <button type="button" aria-label="下一周" @click="calendarOffset += 1"><AppIcon name="chevron-right" :size="18" /></button>
            </div>
          </template>
        </AppSectionHeader>
        <div v-if="visibleCalendarDays.length" class="calendar-week">
          <div v-for="day in visibleCalendarDays" :key="day.date" class="calendar-day" :class="{ 'calendar-day--today': day.isToday }" :data-status="day.status">
