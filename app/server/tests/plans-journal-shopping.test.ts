...
import { addShoppingItem, createShoppingList, generateShoppingList, updateShoppingItem } from '../src/modules/shopping/service.js';
...
describe('节点 5 计划、日记、清单、日历和统计', () => {
  beforeAll(async () => {
    ...
    const records = await database.mealRecord.findMany({ where: { recordDate: { gte: '2040-01-01', lte: '2040-01-31' } }, select: { id: true } });
    ...
    const plans = await database.mealPlan.findMany({ where: { planDate: { gte: '2040-01-01', lte: '2040-01-31' } }, select: { id: true } });
    ...
    const lists = await database.shoppingList.findMany({ where: { name: { startsWith: '节点5测试' } }, select: { id: true } });
  ...
  it('多项目计划完成后只生成一个 DRAFT 记录且重复完成幂等', async () => {
    ...
    const plan = await createPlan(database, {
      planDate: '2040-01-02', mealType: 'DINNER', dinerCount: 2,
      items: [
  ...
  it('计划过期版本经 API 映射为 409', async () => {
    ...
    const response = await app.inject({ method: 'PUT', url: `/api/v1/plans/${plan.id}`, payload: { version: plan.version, notes: '旧设备覆盖' } });
  ...
  it('记录支持草稿确认和组合筛选', async () => {
    const draft = await createRecord(database, {
      recordDate: '2040-01-04', mealType: 'BREAKFAST', sourceType: 'HOMEMADE', status: 'DRAFT',
      rating: 4.5, notes: '燕麦早餐', items: [{ itemType: 'CUSTOM', customName: '燕麦粥' }]
    });
    const confirmed = await confirmRecord(database, draft.id, draft.version);
    const records = await listRecords(database, { from: '2040-01-04', to: '2040-01-04', status: 'CONFIRMED', q: '燕麦' });
  ...
  it('同食材可换算单位合并，不可换算单位保持独立，父版本控制冲突', async () => {
    ...
    list = await addShoppingItem(database, list.id, list.version, { ingredientName: '大米', quantity: 500, unit: 'GRAM' });
    ...
    await expect(updateShoppingItem(database, list.items[0]!.id, 1, { completed: true })).rejects.toMatchObject({ name: 'VersionConflictError' });
  });

  it('低库存生成清单并合并重复来源', async () => {
    const ingredient = await database.ingredient.create({ data: { name: '节点5低库存鸡蛋', quantity: 1, minStock: 6, unit: 'PIECE' } });
  ...
  it('日历同日保留计划和确认记录双标记，草稿单独标记', async () => {
    ...
    await createRecord(database, { recordDate: '2040-01-05', mealType: 'LUNCH', sourceType: 'DINE_IN', items: [{ itemType: 'CUSTOM', customName: '午饭' }] });
    await createRecord(database, { recordDate: '2040-01-05', mealType: 'BREAKFAST', sourceType: 'HOMEMADE', status: 'DRAFT', items: [{ itemType: 'CUSTOM', customName: '早餐' }] });
  ...
  it('统计只计算 CONFIRMED 记录', async () => {
    await createRecord(database, { recordDate: '2040-01-06', mealType: 'LUNCH', sourceType: 'TAKEOUT', status: 'CONFIRMED', rating: 5, isNewTry: true, items: [{ itemType: 'CUSTOM', customName: '确认餐' }] });
    await createRecord(database, { recordDate: '2040-01-06', mealType: 'DINNER', sourceType: 'TAKEOUT', status: 'DRAFT', rating: 1, items: [{ itemType: 'CUSTOM', customName: '草稿餐' }] });
