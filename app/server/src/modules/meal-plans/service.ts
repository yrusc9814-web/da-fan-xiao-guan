...
export async function listPlans(database: PrismaClient, query: { from?: string; to?: string; status?: PlanStatus }) {
  return database.mealPlan.findMany({
    where: {
      deletedAt: null,
      ...(query.from || query.to ? { planDate: { ...(query.from ? { gte: query.from } : {}), ...(query.to ? { lte: query.to } : {}) } } : {}),
...
export async function createPlan(database: PrismaClient, input: PlanInput) {
  ...
  return database.$transaction(async (tx) => {
    const existing = await tx.mealPlan.findUnique({ where: { planDate_mealType: { planDate: input.planDate, mealType: input.mealType } } });
    ...
    if (existing) {
      ...
      await tx.mealPlan.update({
        ...
        data: {
          dinerCount: input.dinerCount, status, notes: input.notes ?? null, deletedAt: null, completedAt: null,
          version: { increment: 1 },
...
export async function updatePlan(database: PrismaClient, id: string, version: number, input: Partial<PlanInput>) {
  return database.$transaction(async (tx) => {
    ...
    if (current.version !== version) throw new VersionConflictError({ entity: 'MealPlan', id, expectedVersion: version, actualVersion: current.version });
    if (current.status === 'COMPLETED') throw httpError(409, '已完成计划不能修改');
    if (current.status === 'CANCELLED' && input.status !== 'CANCELLED') throw httpError(409, '已取消计划不能恢复为其他状态');
...
export async function deletePlan(database: PrismaClient, id: string, version: number) {
  return database.$transaction(async (tx) => {
    ...
    const deletedAt=new Date();await tx.mealPlan.update({ where: { id }, data: { deletedAt, version: { increment: 1 } } });
...
export async function completePlan(database: PrismaClient, id: string, version: number) {
  return database.$transaction(async (tx) => {
    const current = await tx.mealPlan.findFirst({ where: { id, deletedAt: null }, include: { items: true, diners: true } });
    if (!current) throw httpError(404, '计划不存在');
    const existingDraft = await tx.mealRecord.findUnique({ where: { sourceMealPlanId: id }, include: { items: true, diners: true } });
    if (current.status === 'COMPLETED' && existingDraft) return { plan: current, record: existingDraft, idempotent: true };
    if (current.version !== version) throw new VersionConflictError({ entity: 'MealPlan', id, expectedVersion: version, actualVersion: current.version });
    ...
    const record = existingDraft ?? await tx.mealRecord.create({
      data: {
        ...
        relatedPlanId: id,
        sourceMealPlanId: id,
        items: { create: current.items.map((item) => ({ itemType: item.itemType, mealRole: item.mealRole, recipeId: item.recipeId, storeId: item.storeId, customName: item.customName, sortOrder: item.sortOrder })) },
        diners: { create: current.diners.map((link) => ({ dinerId: link.dinerId })) }
      },
      include: { items: true, diners: true }
    });
    const plan = await tx.mealPlan.update({ where: { id }, data: { status: 'COMPLETED', completedAt: new Date(), version: { increment: 1 } }, include: planInclude });
