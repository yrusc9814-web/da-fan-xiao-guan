...
export async function registerIngredientRoutes(app: FastifyInstance, database: PrismaClient): Promise<void> {
  ...
  app.post<{ Body: IngredientWriteInput }>('/api/v1/ingredients', async (request, reply) =>
    reply.code(201).send(success(await createIngredient(database, request.body)))
  );
  ...
  app.delete<{ Params: { id: string }; Querystring: { version: number } }>('/api/v1/ingredients/:id', async (request) =>
    success(await deleteIngredient(database, request.params.id, Number(request.query.version)))
  );
  app.post<{ Params: { id: string }; Body: BatchAdjustmentInput }>('/api/v1/ingredients/:id/adjust', async (request) =>
    success(await adjustInventory(database, request.params.id, request.body))
  );
