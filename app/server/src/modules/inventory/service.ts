...
export async function recommendFromInventory(database: PrismaClient, input: InventoryRecommendationInput) {
  ...
  const batches = await database.inventoryBatch.findMany({
    where: {
      deletedAt: null, quantity: { gt: 0 }, ingredient: { deletedAt: null },
  ...
  const wasteIds = new Set(batches.filter((batch) =>
    batch.consumePriority || batch.opened || Boolean(batch.expiryDate && batch.expiryDate >= today && batch.expiryDate <= in3Days)
  ...
  const candidates = recipes.map((recipe) => {
    const existing: Array<{ ingredientId: string; name: string; required: number | null; available: number; unit: QuantityUnit | null }> = [];
    const missing: Array<{ ingredientId: string | null; name: string; quantity: number | null; unit: QuantityUnit | null; reason: string }> = [];
    let requiredCount = 0;
    ...
    for (const item of recipe.ingredients) {
      ...
      if (!item.ingredientId || item.quantity == null || item.unit == null) {
        missing.push({ ingredientId: item.ingredientId, name: item.ingredientNameSnapshot, quantity: item.quantity, unit: item.unit, reason: !item.ingredientId ? '未关联库存食材' : '缺少可计算的数量或单位' });
        continue;
      }
      const available = availableQuantity(stock, item.unit);
      if (!available.convertible || available.quantity < item.quantity) {
        missing.push({ ingredientId: item.ingredientId, name: item.ingredientNameSnapshot, quantity: Math.max(0, item.quantity - available.quantity), unit: item.unit, reason: available.convertible ? '库存不足' : '库存单位不可换算' });
      } else {
        satisfiedCount += 1;
        existing.push({ ingredientId: item.ingredientId, name: item.ingredientNameSnapshot, required: item.quantity, available: available.quantity, unit: item.unit });
      }
    ...
    const missingTools = recipe.tools.filter((link) => link.required && (!link.tool || link.tool.deletedAt || link.tool.quantity <= 0 || ['BROKEN', 'MISSING', 'UNAVAILABLE'].includes((link.tool.status ?? '').toUpperCase()))).map((link) => link.toolNameSnapshot);
    const completion = requiredCount === 0 ? 100 : Math.round((satisfiedCount / requiredCount) * 100);
    const consumesRequested = requested.size === 0 || recipe.ingredients.some((item) => item.ingredientId && requested.has(item.ingredientId));
    return {
      recipe: { id: recipe.id, name: recipe.name, imagePath: recipe.imagePath, cookingTimeMinutes: recipe.cookingTimeMinutes },
      existingIngredients: existing, missingIngredients: missing, missingTools, completion,
      completionLabel: completion === 100 ? '100% 可直接做' : missing.length <= 2 ? '缺少 1–2 项' : '缺少多项',
      wasteScore, consumesRequested,
      reason: completion === 100 ? (wasteScore > 0 ? '现有库存可完成，并可优先消耗临期或已开封食材' : '现有库存可直接完成') : `还缺少 ${missing.length} 项食材`
    };
    ...
    .filter((candidate) => input.mode !== 'MUST_CONSUME' || (requested.size > 0 ? candidate.consumesRequested : candidate.wasteScore > 0))
    .sort((a, b) => input.mode === 'MUST_CONSUME' ? b.wasteScore - a.wasteScore || b.completion - a.completion : b.completion - a.completion || b.wasteScore - a.wasteScore);

  return { mode: input.mode, candidateCount: candidates.length, items: candidates.slice(0, Math.min(100, Math.max(1, input.limit ?? 20))) };
}

export async function listAllInventoryLogs(database: PrismaClient, query: { ingredientId?: string; batchId?: string; take?: number } = {}) {
  return database.inventoryLog.findMany({
    where: { ingredientId: query.ingredientId, inventoryBatchId: query.batchId },
    orderBy: { createdAt: 'desc' }, take: Math.min(100, Math.max(1, query.take ?? 50))
  });
}
