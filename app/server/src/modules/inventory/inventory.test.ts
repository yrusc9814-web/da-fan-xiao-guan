  ...
  it('按批次库存换算、工具硬过滤和剩菜优先排序', async () => {
    ...
    const recipe = await database.recipe.create({
      data: {
        ...
        ingredients: {
          create: { ingredientId: ingredient.id, ingredientNameSnapshot: ingredient.name, quantity: 300, unit: 'GRAM' }
        },
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/v1/kitchen/recommend',
      payload: { mode: 'ONLY_INVENTORY' }
    });
    ...
    expect(blocked.json().data.items.some((item: { recipe: { id: string } }) => item.recipe.id === recipe.id)).toBe(
      false
    );
  });
});
