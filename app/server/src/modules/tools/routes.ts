...
export async function registerToolRoutes(app: FastifyInstance, database: PrismaClient): Promise<void> {
  ...
  app.post<{ Body: ToolWriteInput }>('/api/v1/tools', async (request, reply) =>
  ...
  app.delete<{ Params: { id: string }; Querystring: { version: number } }>('/api/v1/tools/:id', async (request) =>
    success(await deleteTool(database, request.params.id, Number(request.query.version)))
  );
}
