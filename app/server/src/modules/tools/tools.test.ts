...
describe('厨房工具 API', () => {
  beforeAll(async () => { await database.$connect(); registerErrorHandlers(app); await registerToolRoutes(app, database); });
  afterAll(async () => { await app.close(); await database.$disconnect(); });

  it('支持 CRUD、软删除和版本冲突', async () => {
    const created = await app.inject({ method: 'POST', url: '/api/v1/tools', payload: { name: '测试电饭锅', quantity: 1, status: 'AVAILABLE' } });
    expect(created.statusCode).toBe(201);
    const tool = created.json().data;
    const updated = await app.inject({ method: 'PUT', url: `/api/v1/tools/${tool.id}`, payload: { version: tool.version, name: '测试电饭锅', quantity: 2, status: 'AVAILABLE' } });
    expect(updated.json().data.version).toBe(2);
    const conflict = await app.inject({ method: 'PUT', url: `/api/v1/tools/${tool.id}`, payload: { version: 1, name: '冲突', quantity: 1 } });
